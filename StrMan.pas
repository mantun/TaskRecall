...
TStringManager
 ...
 Compiler:  Delphi4, Delphi5, Delphi6, Delphi7, (Delphi 9 not properly tested)
  Version:  2.4r
 Lastdate:  31 Dec 2004
...
Modifications, additions:
 ...
 04 May 2003 Bug fixes to functions; ToC and UnQuote by Donovan J. Edye
  1 Jul 2003 RangeCheck error fix in IsSame, IsSameFirst after report by Aad Mathijssen
 19 Aug 2003 Small changes too TStrBufPtr
 28 Nov 2003 New overloaded Trim function
 11 Mar 2004 New (faster) overloaded FormatSize function
 20 Mar 2004 New compare function
 22 Nov 2004 New function AddEnsureDelim
 12 Dec 2004 Improved VisualStrToCharSet
===============================================================================}
...
TStringManager = class //Global instance "sm" in initialization section
  ...
  function  Trim(const s : String; aTrimSet : TCharSet; DoUpcase : Boolean = False) : String; overload;
  ...
  //Compare functions-----------------------------------------------------------
  function  Compare(const s1, s2 : String; IgnoreCase : Boolean) : Integer;
  ...
  procedure AddEnsureDelim(var s : String; const AddValue, aDelim : String);
  ...
  function  FormatSize(Value : Integer) : String; overload;  
  function  FormatSize(Value : Int64) : String; overload;
  ...
  function  VisualStrToCharSet(const s: String): TCharSet;
...
type
  ...
  public
    ...
    procedure Truncate;
    procedure Add(Value : Char; Delta : Integer = 16); overload;
    procedure Add(const Value : String; Delta : Integer = 16); overload;
    ...
    procedure TrimRight(DoUpcase : Boolean = False; DoTruncate : Boolean = True);
  end;
  ...
  public
    ...
    procedure Add(Value : Char; Delta : Integer = 16); overload;
    procedure Add(const Value : String; Delta : Integer = 16); overload;
    procedure AddEnsureDelim(const Value, aDelim : String; Delta : Integer = 16);
    procedure Truncate;
    property  Len : Integer read FCharLen write SetCharLen;
    property  MemLen : Integer read GetMemLen write SetMemLen;
    ...
    function  FirstChar : Char;
    function  LastChar : Char;
    procedure Trim(DoUpcase : Boolean = False; DoTruncate : Boolean = True);
    procedure TrimRight(DoUpcase : Boolean = False; DoTruncate : Boolean = True);
  end;
...
{-------------------------------------------------------------------------------
*MopUp - Mop treats control characters as spaces and then removes leading/trailing
...
See also TrimRight, Strip, Mop and the overloaded Set function
-------------------------------------------------------------------------------}
...
{-------------------------------------------------------------------------------
*Trim - Strips a specified set of leading/trailing characters from a string.
Example:
   Trim('.,.hello,,world,,.', ['.',',']) > 'hello,,world'
See also TrimRight, Strip, Mop and the default function above
-------------------------------------------------------------------------------}
function TStringManager.Trim(const s : String; aTrimSet : TCharSet; DoUpcase : Boolean = False) : String;
var i, l: Integer;
begin
  l := Length(s);
  i := 1;
  while (i <= l) and (s[i] in aTrimSet) do Inc(i);
  if i > l then Result := '' else
  begin
    while s[l] in aTrimSet do System.Dec(l);
    Result := System.Copy(s, i, l - i + 1);
  end;
  if DoUpCase
    then for i := 1 to Length(Result) do Result[i] := gaANSIUpperArray[Result[i]];
...
procedure TStringManager.AddEnsureDelim(var s : String; const AddValue, aDelim : String);
{
  to get 'a;b;c;d'
  use AddEnsureDelim(s, 'a', ';');  AddEnsureDelim(s, 'b', ';'); etc
}
begin
  if s=''
    then s := AddValue
    else s := s+aDelim+AddValue;
end;


{-------------------------------------------------------------------------------
*Compare - Combination of the Windows AnsiCompareStr and AnsiCompareText functions.

 Returns 0 if s1=s2, >1 if s1>s2 else <1

  1 million calls on a Pentium 1.1 Ghz in milliseconds:
    SysUtils    CompareText  40  (simple ascii comparison)
    This            Compare  60
    Windows AnsiCompareText 651
-------------------------------------------------------------------------------}
function TStringManager.Compare(const s1, s2 : String; IgnoreCase : Boolean) : Integer;
var  i, aUpto : SmallInt;
 aLen1, aLen2 : SmallInt;
begin
  aLen1 := Length(s1);
  aLen2 := Length(s2);
  ...
  if aLen1>0 then
  begin
    aUpto := aLen1; if aUpto>aLen2 then aUpto := aLen2;
    if IgnoreCase then
    begin
      for i := 1 to aUpto do
      begin
        if gaANSIUpperArray[s1[i]]<>gaANSIUpperArray[s2[i]] then
        begin
          Result := Byte(gaANSIUpperArray[s1[i]])-Byte(gaANSIUpperArray[s2[i]]);
          Exit;
        end;
      end;
    end else
...
*FormatSize - Integer to string with thousand separator.
 Example: FormatSize(23000) > '23,000'
          FormatSize(-1334483648) > '-1,334,483,648'
-------------------------------------------------------------------------------}
function TStringManager.FormatSize(Value : Integer) : String;
const Key = '0123456789';
   MaxLen = 14; //max length it can be with a negative sign
var x : Integer;
 l, n : Byte;
 aNeg : Boolean;
begin
  ...
  repeat
    if n>0 then if n mod 3 =0 then begin Result[l] := ThousandSeparator; Dec(l); end;
    Inc(n);
    x := Value mod 10;
    Value := Value div 10;
    Result[l] := Key[x+1]; Dec(l);
  until Value=0;
  if aNeg then begin Result[l] := '-'; Dec(l); end;
  Delete(Result, 1, l);
end;
...
System locale they are presented in their Byte value with a hash prefix.
...
{-------------------------------------------------------------------------------
VisualStrToCharSet - Converts a string in visual TCharSet form back to a TCharSet
Note that the characters in the string do not have single quotes around them,
and that 'evz' is equivalent to 'e,v,z'.
Example:
  VisualStrToCharSet('A..Z, abcdx') >  ['A'..'Z','a'..'d','x']
  VisualStrToCharSet('#32,A..#68,abce,k..r,#170..#175') > [#32,'A'..'D','a'..'c','e','k'..'r',#170..#175];

  These characters must be entered in ordinal form: (#xxx)
    #0..#32        - control characters and space
    #35, #44, #46  - #,.
See also CharSetToVisualStr
-------------------------------------------------------------------------------}
function TStringManager.VisualStrToCharSet(const s : String) : TCharSet;
var p : PChar;

    function GrabOrdinal: Byte; //like #65
    begin
      Inc(p);
      Result := 0;
      while (p^<>#0) and (p^<=' ') do Inc(p); //eat spaces
      while (p^ in ['0'..'9']) do
      begin
...
begin
  ...
  begin
    ...
    #1..#32 : Inc(p);
      ...
      begin
        if p^='#' then aLeft := GrabOrdinal else begin aLeft := Byte(p^); Inc(p); end;
        if p^='.' then
        begin
          Inc(p); if p^<>'.' then Break else Inc(p);
          while (p^<>#0) and (p^<=' ') do Inc(p); //eat spaces
          if p^ in [#0, ',','.'] then Break;
          if p^='#' then aRight := GrabOrdinal else begin aRight := Byte(p^); Inc(p); end;
          if aLeft>aRight then Break;
          for aLoop := aLeft to aRight do Include(Result, Char(aLoop));
        end else
        begin
          Include(Result, Char(aLeft));
        end;
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    if f.Size>0 then f.Read(Result[1], Length(Result));
  finally
    f.Free;
  end;
end;
...
begin
  ...
  if aLen>0 then aStream.WriteBuffer(aText[1], aLen);
end;
...
{See notes above---------------------------------------------------------------}
procedure TStrBufPtr.Add(Value : Char; Delta : Integer = 16);
...
{See notes above---------------------------------------------------------------}
procedure TStrBufPtr.Add(const Value : String; Delta : Integer = 16);
var Allocated : Integer;
         SLen : Integer;
         TPos : Integer;
begin
...
procedure TStrBufPtr.TrimRight(DoUpcase : Boolean = False; DoTruncate : Boolean = True);
var i : Integer;
begin
  while (FCharLen>0) and (Fp^[FCharLen] <=  ' ') do System.Dec(FCharLen);
  if DoTruncate then Truncate;
  if DoUpcase then for i := 1 to FCharlen do Fp^[i] := gaANSIUpperArray[Fp^[i]];
end;
...
procedure TStrBuf.Add(Value : Char; Delta : Integer = 16);
var Allocated : Integer;
begin
  Allocated := Length(Fs);
...
procedure TStrBuf.Add(const Value : String; Delta : Integer = 16);
var Allocated : Integer;
         SLen : Integer;
         TPos : Integer;
begin
...
procedure TStrBuf.AddEnsureDelim(const Value, aDelim : String; Delta : Integer = 16);
{
  to get 'a;b;c;d'
  use AddEnsureDelim('a', ';');  AddEnsureDelim('b', ';'); etc
}
begin
  if Len=0
    then Add(Value, Delta)
    else Add(aDelim+Value, Delta);
end;
...
procedure TStrBuf.Trim(DoUpcase : Boolean = False; DoTruncate : Boolean = True);
var i, l: Integer;
begin
  l := FCharLen;
  i := 1;
...
procedure TStrBuf.TrimRight(DoUpcase : Boolean = False; DoTruncate : Boolean = True);
var i : Integer;
begin
